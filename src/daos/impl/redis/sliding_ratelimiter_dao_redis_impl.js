...
const hitSlidingWindow = async (name, opts) => {
  ...
  const key = keyGenerator.getSlidingWindowRateLimiterKey(name, opts.interval, opts.maxHits);

  const transaction = client.multi();
  const score = timeUtils.getCurrentTimestampMillis();
  const value = `${score}-${Math.random()}`;
  transaction.zadd(key, score, value);
  transaction.zremrangebyscore(key, 0, (score - opts.interval));
